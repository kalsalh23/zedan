// Redeploys the img function with robust path extraction
const fs = require('fs')
...
const imgFn = `// img: serve self-hosted images with immutable caching — any subpath after '/img/'
Deno.serve(async (req: Request) => {
  ...
  p = p.replace(/\\/+$/, '')
  if (!p) return new Response('not found: ' + raw, { status: 404 })
  ...
  const row = rows?.[0]
  if (!row) return new Response('not found: ' + p, { status: 404 })
